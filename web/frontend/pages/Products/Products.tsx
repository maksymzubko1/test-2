...
import {
  queryProductsAppsGet,
  queryProductsMarketsGet,
  queryProductsGet,
} from "./requests";
...
export const Products = () => {
  ...
  const { data: dataApps, isLoading: isLoadingApps } =
    useGetProductsApps(options);
  const { data: dataMarkets, isLoading: isLoadingMarkets } =
    useGetProductsMarkets(options);

  const { show: showToast } = useToast();

  useEffect(() => {
    if (isError && !isLoading)
      showToast("Error fetch products list", { isError: true });
  }, [isError, isLoading]);
    ...
    <Page fullWidth>
      <VerticalStack gap={"12"}>
        <HorizontalStack align={"space-between"} blockAlign={"center"}>
          ...
          <Button primary onClick={() => navigate(`${E_Routes.products}/new`)}>
            Add product
          </Button>
        </HorizontalStack>
        <ProductsTable
          data={{ dataApps, dataMarkets, allData: data }}
          loadings={{
            allDataLoading: isLoading,
            dataMarketsLoading: isLoadingMarkets,
            dataAppsLoading: isLoadingApps,
          }}
